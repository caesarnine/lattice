...
export function ThreadSidebar({
  ...
  React.useEffect(() => {
    if (isAgentOpen && onLoadAgents) {
      onLoadAgents();
    }
  }, [isAgentOpen, onLoadAgents]);
  ...
  const displayAgent = agentName || agent || defaultAgent || "Default";
  const normalizedAgentQuery = agentQuery.trim().toLowerCase();
  const allAgents = agents ?? [];
  const filteredAgents = normalizedAgentQuery
    ? allAgents.filter(
        (item) =>
          item.name.toLowerCase().includes(normalizedAgentQuery) ||
          item.id.toLowerCase().includes(normalizedAgentQuery)
      )
    : allAgents;
  const visibleAgents = normalizedAgentQuery ? filteredAgents : filteredAgents.slice(0, 50);
    ...
    <aside className="flex h-full flex-col border-ink-200/70 bg-glass shadow-soft md:rounded-3xl md:border overflow-hidden">
      <div className="space-y-4 px-5 pb-4 pt-6">
        ...
        <div className="rounded-2xl border border-ink-200/70 bg-white/60 p-3">
          ...
          >
            <span>Agent</span>
            <span className="max-w-[140px] truncate text-ink-900 normal-case" title={displayAgent}>
              {displayAgent}
            </span>
          </button>
          {isAgentOpen ? (
            <div className="mt-3 space-y-3">
                ...
                <ScrollArea className="h-56 px-2 py-2">
                  {isLoadingAgents ? (
                    ...
                    visibleAgents.map((item) => {
                      const isSelected = item.id === agent;
                      return (
                        <button
                          key={item.id}
                          type="button"
                        ...
                        >
                          ...
                          <div className="mt-0.5 truncate text-[10px] text-ink-400">{item.id}</div>
                        </button>
                      );
                    })
                  ) : (
              ...
              {!normalizedAgentQuery && allAgents.length > visibleAgents.length ? (
                <div className="text-[11px] text-ink-400">
                  Showing {visibleAgents.length} of {allAgents.length}. Type to search.
                </div>
              ) : null}
              {defaultAgent && agent && agent !== defaultAgent ? (
                <Button
                  size="sm"
                  variant="outline"
