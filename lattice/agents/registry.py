...
class AgentRegistry:
    ...
    def resolve_id(self, requested: str, *, allow_fuzzy: bool = True) -> str | None:
        ...
        by_name = [agent_id for agent_id, plugin in self.agents.items() if plugin.name.casefold() == needle]
        if len(by_name) == 1:
            return by_name[0]
        if len(by_name) > 1:
            return None
        ...
        prefix_by_name = [
            agent_id
            for agent_id, plugin in self.agents.items()
            if plugin.name.casefold().startswith(needle)
        ]
        if len(prefix_by_name) == 1:
            return prefix_by_name[0]

        prefix_by_id = [agent_id for agent_id in self.agents.keys() if agent_id.casefold().startswith(needle)]
...
def discover_builtin_agent_specs() -> list[str]:
    try:
        pkg = importlib.import_module("lattice.agents.builtins")
    except Exception:
        return []
...
def _resolve_default_agent_id(
    ...
    needle = value.casefold()
    exact = [agent_id for agent_id, plugin in agents.items() if plugin.name.casefold() == needle]
    if len(exact) == 1:
        return exact[0]
    if len(exact) > 1:
        return None

    prefix = [agent_id for agent_id, plugin in agents.items() if plugin.name.casefold().startswith(needle)]
    if len(prefix) == 1:
        return prefix[0]
    return None
...
def load_registry(
    *,
    ...
    resolved_default = (
        _resolve_default_agent_id(agents, configured_default) if configured_default else None
    )
