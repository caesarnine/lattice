...
from lattice.protocol.models import TranscriptMessage
...
def _transcript_from_request(message: ModelRequest) -> list[TranscriptMessage]:
    ...
    for part in message.parts:
        if isinstance(part, SystemPromptPart):
            ...
            items.append(TranscriptMessage(role="user", content=_stringify_user_content(part.content)))
    return items
...
def _transcript_from_response(message: ModelResponse) -> list[TranscriptMessage]:
    ...
    for part in message.parts:
        if isinstance(part, (ToolReturnPart, BuiltinToolReturnPart)):
            items.append(TranscriptMessage(role="tool", content=_stringify_value(part.content)))
    return items
