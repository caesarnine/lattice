...
def resolve_storage_config(
    *,
    ...
    db_path = _coerce_path(db_path) if db_path is not None else data_dir / "lattice.db"
    session_id_path = (
...
def resolve_storage_config_from_env(
    *,
    ...
    env_workspace_mode = read_env(LATTICE_WORKSPACE_MODE)
    env_data_dir = read_env(LATTICE_DATA_DIR)
    env_workspace_dir = read_env(LATTICE_WORKSPACE_DIR)
    env_db_path = read_env(LATTICE_DB_PATH)
    env_session_path = read_env(LATTICE_SESSION_FILE)
